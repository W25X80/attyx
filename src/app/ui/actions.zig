...
pub fn processSplitActions(ctx: *PtyThreadCtx) void {
    ...
    // When zoomed, only allow zoom toggle and pane close
    if (layout.isZoomed()) {
        switch (action) {
            .pane_zoom_toggle => {},
            .pane_close => {},
    ...
    switch (action) {
        ...
        .pane_zoom_toggle => {
            ...
            const pty_rows: u16 = @intCast(@max(1, @as(i32, ctx.grid_rows) - terminal.g_grid_top_offset - terminal.g_grid_bottom_offset));
            if (layout.isZoomed()) {
                // Resize zoomed pane to fill entire area
                const pane = layout.focusedPane();
                pane.resize(pty_rows, ctx.grid_cols);
            } else {
...
pub const computeSplitGaps = split_actions_mod.computeSplitGaps;
...
fn doSplit(ctx: *PtyThreadCtx, layout: *SplitLayout, dir: split_layout_mod.Direction) void {
    split_actions_mod.doSplit(ctx, layout, dir);
}
