...
- (void)insertText:(id)string replacementRange:(NSRange)replacementRange {
    ...
    if (utf8 && strlen(utf8) > 0) {
        ...
        } else if (g_session_picker_active || g_command_palette_active || g_theme_picker_active || g_tab_picker_active || g_agent_dashboard_active) {
            const uint8_t* p = (const uint8_t*)utf8;
            const uint8_t* end = p + strlen(utf8);
            while (p < end) {
                uint32_t cp = 0;
...
- (void)doCommandBySelector:(SEL)selector {
    ...
    if (g_search_active || g_ai_prompt_active || g_session_picker_active || g_command_palette_active || g_theme_picker_active || g_tab_picker_active || g_agent_dashboard_active) return;
